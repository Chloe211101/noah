...
def load_data():
    try:
        ...
        st.error("❌ 找不到订单数据.xlsx文件，请确保文件在桌面目录下")
        return pd.DataFrame()
...
if df.empty:
    st.warning("⚠️ 请将订单数据.xlsx文件放在桌面目录下")
    st.stop()
...
# 订单创建日期筛选
if date_range_order and len(date_range_order) == 2:
...
# 订单发货日期筛选
if date_range_ship and len(date_range_ship) == 2:
    start_ship = pd.to_datetime(date_range_ship[0])
    end_ship = pd.to_datetime(date_range_ship[1])
    filtered_df = filtered_df[
        (filtered_df['出货日期'] >= start_ship) & 
        (filtered_df['出货日期'] <= end_ship)
    ]
...
st.markdown("---")
st.caption("💡 修改Excel后点击右上角的【刷新数据】按钮即可更新看板 | 所有筛选器可以组合使用，汇总统计会自动更新")