...
def load_data():
    try:
        # 直接从 GitHub 仓库读取 Excel（使用 raw 链接）
        ...
        st.error(f"❌ 读取文件出错：{e}")
        st.info("请确保 GitHub 仓库中存在 订单数据.xlsx 文件")
        return pd.DataFrame()

df = load_data()

if df.empty:
    st.warning("⚠️ 请将订单数据.xlsx 上传到 GitHub 仓库：Chloe211101/noah")
    st.stop()
...
with col3:
    monthly_contract = df[(df['订单日期'].dt.year == current_year) & (df['订单日期'].dt.month == current_month)]['总金额'].sum()
